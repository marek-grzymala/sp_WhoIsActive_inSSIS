...
SELECT   TOP 1 
        ...
        ,CAST(DATEADD(SECOND, PERCENTILE_CONT(@PercentileLower) WITHIN GROUP (ORDER BY DATEDIFF(SECOND, CAST(CAST(ei.start_time AS DATE) AS DATETIME), CAST(ei.start_time AS DATETIME))) OVER (PARTITION BY @PackageName), 0) AS TIME(0)) AS [TimeStart_PercLower]
        ,CAST(DATEADD(SECOND, PERCENTILE_CONT(@PercentileUpper) WITHIN GROUP (ORDER BY DATEDIFF(SECOND, CAST(CAST(ei.start_time AS DATE) AS DATETIME), CAST(ei.start_time AS DATETIME))) OVER (PARTITION BY @PackageName), 0) AS TIME(0)) AS [TimeStart_PercUpper]
        
        ,CAST(DATEADD(SECOND, PERCENTILE_CONT(@PercentileLower) WITHIN GROUP (ORDER BY DATEDIFF(SECOND, CAST(CAST(ei.end_time AS DATE) AS DATETIME), CAST(ei.end_time AS DATETIME))) OVER (PARTITION BY @PackageName), 0) AS TIME(0)) AS [TimeEnd_PercLower]
        ,CAST(DATEADD(SECOND, PERCENTILE_CONT(@PercentileUpper) WITHIN GROUP (ORDER BY DATEDIFF(SECOND, CAST(CAST(ei.end_time AS DATE) AS DATETIME), CAST(ei.end_time AS DATETIME))) OVER (PARTITION BY @PackageName), 0) AS TIME(0)) AS [TimeEnd_PercUpper]
